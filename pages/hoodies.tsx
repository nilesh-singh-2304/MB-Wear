...
import Hood from "../photos/hood.png"
...
const hoodies = ({products}) => {
  return (
    <div>
      <Head>
      <title>{`Hoodies - MBwear.com`}</title>
    </Head>
        ...
        <div className=' mt-8 justify-center items-center '>Get Hoodies with your name <br/><br/> <Image width={500} height={500} src={Hood} alt=''/> </div>
        </div>
      ...
      <div className=" py-6 sm:py-8 lg:py-12">
        <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
          ...
          <div className="grid gap-x-4 gap-y-8 sm:grid-cols-2 md:gap-x-6 lg:grid-cols-3 xl:grid-cols-4">
            {Object.keys(products).length === 0 && <div className="w-screen font-extrabold pr-20 text-center">Currently all the hoodies are out of stock !! Stay Tuned for upcoming stock !!</div>}
...
export async function getServerSideProps(context){
  ...
  let hood = {}
      for(let item of products){
        if(item.title in hood){
          ...
          hood[item.title] = JSON.parse(JSON.stringify(item))     //initially hood empty h so hmne hood with title=item.title m item ko deep copy kr dio
          if(item.availableQty >0){                                 //agr item ki available qty 0 is zda h toh hood with title=item.title k color ko ek array bna do and usk ander item.color ko add krdo and same with size
  ...
  return{
    props:{products: JSON.parse(JSON.stringify(hood))} // will be passed to page components as props
